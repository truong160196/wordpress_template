...
function thinkup_input_blogtitle() {
    ...
    echo '<a href="' . esc_url( get_permalink() ) . '" title="' . esc_attr( sprintf( __( 'Permalink to %s', 'experon' ), the_title_attribute( 'echo=0' ) ) ) . '">',
         '<h4>' . get_the_title() .'</h4>',
        '</a>';
}
...
global $post;
	...
	$link_image  = $featured_img[0];

	// Output media on blog page
	if ( ! empty( $featured_id ) ) {
        $output .= '<a href="'.  get_permalink($post->ID) . '">';
        $output .= '<div class="top-recipe-img set-bg" data-setbg="' . $link_image .'">';
